...
import { useAddPharmacyService } from "@/services/pharmacies/use-add-pharmacy";
...
export const AddPharmacyDrawer = () => {
  const { isOpen, onClose } = useAddPharmacyStore();
  const addPharmacyMutation = useAddPharmacyService();
  ...
  const onSubmit = async (data: PharmacyFormValues) => {
    try {
      await addPharmacyMutation.mutateAsync(data);
    ...
    <Drawer direction="right" open={isOpen} onOpenChange={onClose}>
      <DrawerContent className="right-2 top-2 bottom-2 fixed flex data-[vaul-drawer-direction=right]:sm:max-w-lg bg-gradient-to-b from-white to-gray-50">
        ...
        <DrawerFooter className="flex flex-col justify-end gap-3 border-t pt-4">
          ...
          >
            {addPharmacyMutation.isPending ? "Creating..." : "Add Pharmacy"}
          </Button>
          <DrawerClose asChild>
