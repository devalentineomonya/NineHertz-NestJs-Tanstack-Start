...
import { useEditOrderService } from "@/services/order/use-edit-order";
import { useGetOrder } from "@/services/order/use-get-order";
...
import {
  ...
  MedicineType,
  orderSchema,
} from "@/screens/order/order-schema";

export function EditOrderDrawer() {
  const [total, setTotal] = useState(0);
  const { isOpen, onClose, orderId } = useEditOrderStore();
  const { data: order, isLoading: loadingOrder } = useGetOrder(orderId || "");
  const { data: patients, isLoading: loadingPatients } = useGetPatients();
  const { data: medicinesData } = useGetMedicines();
  const medicines = medicinesData?.data;
  const editOrderMutation = useEditOrderService();
  ...
  const handleSubmit: SubmitHandler<OrderFormValues> = async (data) => {
    ...
    try {
      await editOrderMutation.mutateAsync({
        id: orderId,
        data: { ...data, totalAmount: parseFloat(data.totalAmount.toString()) },
      });
    } catch (error) {
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {fields.map((field, index) => {
                  ...
                  const price = form.watch(`items.${index}.pricePerUnit`) || 0;
                  const lineTotal =
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-[80px] text-right">
                        <p className="text-sm font-medium">Kes {lineTotal}</p>
                      </div>
